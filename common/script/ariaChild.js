...
function ariaAttributeReferences() {
  {
    ...
    Array.prototype.slice.call(document.querySelectorAll("pdef, sdef")).forEach(function (item) {
      ...
      if (container.nodeName.toLowerCase() == "div") {
        ...
        Array.prototype.slice.call(container.attributes).forEach(function (attr) {
    ...
    var subRoles = [];
    var roleIndex = "";
    Array.prototype.slice.call(document.querySelectorAll("rdef")).forEach(function (item) {
      var container = item.parentNode;
      var content = item.innerHTML;
      ...
      var attrs = [];
      Array.prototype.slice.call(container.querySelectorAll(".role-properties, .role-required-properties")).forEach(function (node) {
        if (node && ((node.textContent && node.textContent.length !== 1) || (node.innerText && node.innerText.length !== 1))) {
          // looks like we do
          Array.prototype.slice.call(node.querySelectorAll("pref,sref")).forEach(function (item) {
    ...
    Array.prototype.slice
      ...
      .forEach(function (item) {
        ...
        if (content.length === 1 || content.length === 0) {
          ...
          content === "Placeholder" &&
          !skipIndex &&
          (item.className === "role-inherited" || item.className === "role-children" || item.className === "property-descendants" || item.className === "state-descendants")
        ) {
